  ...
  if (!opt.no_literal)
  ...
  else
    {
      /* User requested not to create a literal packet, so we copy the
         plain data.  */
    byte copy_buffer[16384];
    int  bytes_copied;
    while ((bytes_copied = iobuf_read(inp, copy_buffer, 16384)) != -1)
      if ( (rc=iobuf_write(out, copy_buffer, bytes_copied)) ) {
        log_error ("copying input to output failed: %s\n",
                   gpg_strerror (rc) );
        break;
      }
    wipememory (copy_buffer, 16384); /* burn buffer */
    }
               ...
               int outputfd)
{
  ...
  if (!opt.no_literal)
  ...
  else
    {
      ...
      wipememory (copy_buffer, 16384); /* Burn the buffer. */
    }
