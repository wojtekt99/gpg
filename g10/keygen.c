         ...
         char **cache_nonce_addr, char **passwd_nonce_addr)
{
  ...
  /*
    ...
    16384/256.  Matching sizes is not a very exact science.
         ...
         char **cache_nonce_addr, char **passwd_nonce_addr)
{
  ...
  char *keyparms;
  char nbitsstr[35];
  const unsigned maxsize = (opt.flags.large_rsa ? 16384 : 16384);
  ...
  if (primary_keysize && !opt.expert)
    {
      ...
      if (algo == PUBKEY_ALGO_DSA && primary_keysize > 16384)
        nbits = 16384; /* For performance reasons we don't support more
                         than 16384 bit DSA.  However we won't see this
