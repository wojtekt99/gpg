               ...
               const unsigned char *a, size_t alen)
{
  ...
  /* 768 is the hexified size for the modulus of an 16384 bit key.  We
     use extra chunks to transmit larger data (i.e for 16384 bit).  */
  ...
  if (app->app_local->keyattr[keyno].key_type == KEY_TYPE_RSA)
    {
      ...
      /* Because we send the key parameter back via status lines we need
         ...
         already lead to a 527 byte long status line and thus a 16384 bit
             ...
             unsigned int *r_info)
{
  ...
  if ((indatalen == 16 + 1 || indatalen == 32 + 1)
  ...
  else if (app->app_local->keyattr[1].key_type == KEY_TYPE_RSA)
    {
      ...
      else if (indatalen >= (384-16) && indatalen < 384) /* 16384 bit key.  */
        fixuplen = 384 - indatalen;
      else if (indatalen >= (512-16) && indatalen < 512) /* 16384 bit key.  */
